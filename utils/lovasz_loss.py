...
class Lovasz_loss(nn.Module):
    ...
    def forward(self, probas, labels):
        # probas: [N, num_class] softmax后的概率
        # labels: [N]
...
def flatten_probas(probas, labels, ignore=None):
    ...
    labels = labels.view(-1)
    probas = probas.view(-1, probas.shape[-1])
    if ignore is None:
    ...
    #     return probas, labels
    # valid = (labels != ignore)
    # idx = valid.nonzero(as_tuple=False).squeeze()
    # if idx.numel() == 0:
    #     vprobas = probas.new_zeros((0, probas.size(1)))
    #     vlabels = labels.new_zeros((0,), dtype=labels.dtype)
    # else:
    #     if idx.dim() == 0:
    #         idx = idx.unsqueeze(0)
    #     vprobas = probas[idx]
